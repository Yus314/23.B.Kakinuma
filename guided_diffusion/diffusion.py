...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if simplified:
            print(
                "Run Simplified DDNM, without SVD.",
            ...
            with torch.no_grad():
                ...
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    if j < i:  # normal sampling
                        ...
                        xt_next = (
                            at_next.sqrt() * x0_t
                            + torch.randn_like(x0_t) * (1 - at_next).sqrt()
                        )
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        if deg == "cs_walshhadamard":
            ...
            A_funcs = WalshHadamardCS(
                ...
                compress_by,
                torch.randperm(self.config.data.image_size**2, device=self.device),
                self.device,
            )
        ...
        for x_orig, classes in pbar:
            ...
            if deg[:6] == "deblur":
                ...
                Apy += (
                    A_funcs.A_pinv(A_funcs.A(torch.ones_like(Apy))).reshape(*Apy.shape)
                    - 1
                )
            ...
            with torch.no_grad():
                if sigma_y == 0.0:  # noise-free case, turn to ddnm
                    x, _ = ddnm_diffusion(
                        x,
                        model,
                        self.betas,
